...
function fetchMovies(url, dom_element, path_type) {
  fetch(url) // Use Fetch with the url passed down
    .then((response) => {
      if (response.ok) {
...
showMovies = (movies, dom_element, path_type) => {
  ...
  let moviesEl = document.querySelector(dom_element); // Create a variable that grabs id or class

  for (let movie of movies.results) {
    // Loop through object
    console.log(movie);
    let imageElement = document.createElement("img"); // Within loop create an img element
    ...
    moviesEl.appendChild(imageElement)// Append the imageElement to the dom_element selected
